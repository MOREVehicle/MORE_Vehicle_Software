...
const char dpObj_ThrottleL[] = "j0.val";
const char dpObj_ThrottleR[] = "j1.val";
const char dpObj_ThrottleErr[] = "t5";
...
void NEXTION_SendToPB (char* obj, uint16_t value){
    char buffer[30];
    int len = sprintf(buffer,"%s=%u", obj, value);
    HAL_UART_Transmit(&huart3, (uint8_t*)buffer, len, UART_TIMEOUT);
    HAL_UART_Transmit(&huart3, NextionMSG_END, NT_MSG_END_LENGTH,UART_TIMEOUT);
}
...
void NEXTION_SendErrorMessage(char *objName, char *string){
    char buffer[30];
    int len = sprintf(buffer, "%s.val=\"%s\"", objName, string);
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan){
    ...
    else if(RxHeader.StdId == Throttle_Id && RxHeader.IDE == CAN_ID_STD)
    {
        ...
        if (((WheelThrottleRight < 10) || (WheelThrottleLeft < 10)) || ((WheelThrottleRight > 90) || (WheelThrottleLeft > 90)))
        {
            NEXTION_SendErrorMessage(dpObj_ThrottleErr, "Throttle Connection Error!");
        }
        else
        {

            uint16_t MappedWheelThrottleLeft = ((WheelThrottleLeft - 11) * (100 - 0)) / (90 - 11) + 0;
            uint16_t MappedWheelThrottleRight = ((WheelThrottleRight - 11) * (100 - 0)) / (90 - 11) + 0;
            NEXTION_SendToPB(dpObj_ThrottleL, MappedWheelThrottleLeft);
            NEXTION_SendToPB(dpObj_ThrottleR, MappedWheelThrottleRight);
